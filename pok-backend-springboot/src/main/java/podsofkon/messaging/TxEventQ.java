...
//    SupplierService supplierService;
    ...
    Logger logger = LoggerFactory.getLogger(TxEventQ.class);

//    @JmsListener(destination = "orderuser.orderqueue", containerFactory = "topicConnectionFactory")
//    @JmsListener(destination = "orderuser.orderqueue", containerFactory = "queueConnectionFactory")
    public void listenOrderEvent(String message, AQjmsSession session) throws Exception {   // , selector = "priority = 'low'"
        logger.info("Received Message Session: " + session + " orderMessage :" + message);
        OracleTxEventQJPADataSource.setConnection(session.getDBConnection());
   //     session.getDBConnection().createStatement().execute("INSERT INTO orderuser.testtable VALUES ('test')");
//        updateProperty(Long.valueOf(message));
        logger.info("updated JPA table for orderMessage :" + message);
        if(message.equals("41")) {
            session.rollback();
            throw new Exception("throwing intentional exception as message is 2, check for rollback");
        }
...
//        inventoryEvent(order.getOrderid(), order.getItemid(), location, session);
    ...
    /**
    public ResponseEntity<Properties> updateProperty(long id) {
        try {
                Properties _property = new Properties();
                _property.setId(id);
                _property.setCallcount(44);
                return new ResponseEntity<>(PropertiesController.propertiesRepository.save(_property), HttpStatus.OK);
//            } else {
